...
import { enrollInKtipCourse, KtipEnrollError, ktipApiKey } from '../_lib/ktip-catalog'
...
export default async function handler(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (!supabaseUrl || !serviceKey || !anonKey || !ktipApiKey()) {
    ...
    const { data: limit, error: rateError } = await admin.rpc('consume_auth_rate_limit', {
      p_bucket: `ktip-enroll:user:${caller.id}`,
      p_window_seconds: IP_WINDOW,
      p_limit: IP_LIMIT,
    })
    if (rateError) {
      console.error(`[ktip-enroll] rate limit RPC failed: ${rateError.message}`)
    } else if ((limit as { allowed?: boolean } | null)?.allowed === false) {
    ...
    try {
      ...
      if (err instanceof KtipEnrollError) {
        ...
        console.error(`[ktip-enroll] upstream ${err.status}: ${err.message}`)
        return json({ error: err.message }, 502)
      }
      console.error('[ktip-enroll] unexpected error:', err)
      return json({ error: 'Enrollment failed' }, 502)
    }
  } catch (err) {
    console.error('[ktip-enroll] handler error:', err)
