...
import { DeleteEntityControl } from '../shared/DeleteEntityControl'
import { describeEventDeletion } from '../../lib/delete-guard'
import { DetailsEditor, cleanDetails } from '../shared/DetailsEditor'
import { TagInput } from '../ui/TagInput'
import { CalendarAccentPicker } from '../calendar/CalendarAccentPicker'
...
/**
 * The event fields editor — everything on the events row that the organizer
 * types by hand.
 *
 * Extracted from the old standalone /events/:id/edit page so it can live as
 * the Details tab of the event management workspace: one place to run an
 * event, not an editor here and a console there. The form is unchanged;
 * only the page chrome around it went.
 */
export function EventDetailsForm({
  ...
  eventId: string
  /** Called after a successful save (the workspace refetches its hero). */
  onSaved?: () => void
}) {
  const { t } = useLingui()
  const auth = useAuth()
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const endDatetime = endDate ? combineDatetime(endDate, endTime) : undefined
    ...
    try {
      await updateEvent(eventId, {
        ...
        submission_deadline:
          challengeOn && submissionDate ? combineDatetime(submissionDate, submissionTime) : null,
  ...
  return (
    <div className="max-w-page-tight">
      ...
      {/* Outside the form so Enter in a text field can never reach it.
          The RSVP count is not loaded here, so the guard gets null and falls
          back to the event's publication status. */}
      <div className="mt-10">
        <DeleteEntityControl
          variant="zone"
          noun="event"
