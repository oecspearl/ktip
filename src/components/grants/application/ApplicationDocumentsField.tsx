...
export function ApplicationDocumentsField({
  ...
  const openUpload = async (item: RequiredDocument | null) => {
    if (!applicationId) {
      ...
      try {
        ...
        toast.error('Could not save the draft — try again before uploading')
        setSaving(false)
        return
      }
      setSaving(false)
  ...
  const handleDelete = async (id: string, storagePath: string | null, title: string) => {
    if (!window.confirm(`Remove "${title}" from this application?`)) return
    try {
      await deleteDocument({ documentId: id, storagePath })
      toast.success('Document removed')
    } catch (err: any) {
      toast.error(err?.message || 'Failed to remove the document')
    }
  }
    ...
    <div className="space-y-5">
      <div className="flex items-start gap-2 rounded-xl border border-ktip-ocean-200 bg-ktip-ocean-50/60 p-3">
        ...
        <p className="text-xs text-ktip-ocean-800">
          Everything you attach here is private to you and the people assessing this grant. It is
          not listed on the public grant page, and other applicants cannot see it.
        </p>
      </div>
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <p className="flex items-center gap-1.5 text-sm font-medium text-ktip-sand-900">
                    ...
                    {item.label}
                    {item.required ? (
                      <span className="text-xs font-normal text-ktip-sun-800">Required</span>
                    ) : (
                      <span className="text-xs font-normal text-ktip-sand-500">Optional</span>
                    )}
                  </p>
                ...
                >
                  {found.length > 0 ? 'Add another' : 'Upload'}
                </Button>
              </div>
            </div>
          )
      ...
      <div>
        ...
        {!applicationId ? (
          <p className="rounded-xl border border-dashed border-ktip-sand-300 py-6 text-center text-sm text-ktip-sand-500">
            Your draft is saved automatically the first time you upload.
          </p>
        ) : loading ? (
          <p className="py-6 text-center text-sm text-ktip-sand-500">Loading attachments…</p>
        ) : uploaded.length === 0 ? (
          <p className="rounded-xl border border-dashed border-ktip-sand-300 py-6 text-center text-sm text-ktip-sand-500">
      ...
      {applicationId && (
        <DocumentUploadModal
          ...
          descriptionCopy="Attached to this application only. Visible to you and the grant assessors."
        />
      )}
    </div>
  )
