  ...
  return (
    <Link
      ...
      className={cn(
        ...
        '@container group relative rounded-2xl p-6 flex flex-col justify-between gap-6 overflow-hidden shadow-medium hover:shadow-hard hover:-translate-y-0.5 transition-all duration-300 h-full min-h-[13rem]',
        className
      )}
    >
      <img
      ...
      <div className="relative flex gap-4">
        <div className="min-w-0 flex-1">
          ...
          {visibleTags.length > 0 && (
            <p className="@md:hidden mt-2 text-[10px] font-semibold uppercase tracking-widest text-white/75 [text-shadow:0_1px_6px_rgba(0,0,0,0.3)]">
              {visibleTags.join(' · ')}
        ...
        {visibleTags.length > 0 && (
          <div className="hidden @md:flex w-1/5 shrink-0 flex-col items-end gap-1 text-right">
            {visibleTags.map((tag) => (
              <span
                key={tag}
                className="max-w-full truncate text-[10px] font-semibold uppercase tracking-widest text-white/75 [text-shadow:0_1px_6px_rgba(0,0,0,0.3)]"
              >
                {tag}
              </span>
            ))}
