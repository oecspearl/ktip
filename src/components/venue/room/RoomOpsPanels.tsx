...
/**
 * The two host actions that belong in the room rather than in the editor.
 *
 * Closing a room and announcing something are both things a host does *while*
 * the event is running, standing in the room it applies to. Everything slower
 * than that — renaming, recolouring, moving a wall — stays in the venue
 * builder, where it can be undone before it is saved.
 *
 ...
 * inside venue_room_broadcast().
 */
export function HostControlsPanel({ room }: { room: VenueRoom }) {
  ...
  return (
    <div className="rounded-2xl border border-ktip-ocean-200 bg-ktip-ocean-50/60 p-4">
      ...
      <button
        ...
        onClick={toggleOpen}
        disabled={saving}
        className="mt-3 flex items-center gap-1.5 rounded-lg border border-ktip-sand-200 bg-ktip-cream px-2.5 py-1.5 text-xs font-semibold text-ktip-sand-700 hover:border-ktip-ocean-300 hover:text-ktip-ocean-700 disabled:opacity-50"
      >
        {room.is_open ? <Lock size={13} aria-hidden="true" /> : <Unlock size={13} aria-hidden="true" />}
