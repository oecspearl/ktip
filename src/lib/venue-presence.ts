 ...
 * per room. Supabase Presence hands every subscriber the complete state, so
 * per-room occupancy is a client-side groupBy — which is why a floorplan with
 * nine rooms costs one subscription instead of nine.
 *
 * Everything here is a pure function of (presence state, DB roster, now) so it
 * can be unit-tested without a socket. The hook in useVenuePresence.ts owns the
 ...
 * presence_ref-tracking client, each holding an array of tracked payloads.
 * Typed loosely on purpose — this module must not import the realtime client.
 */
 ...
 * disturb" set in one window must never be softened to "working" because
 * another window is idle-reporting. 'help_wanted' outranks 'working' for the
 * same reason in the opposite direction — an unanswered ask for help is the
 * one status the venue exists to surface.
 */
 ...
 * union — a member reading the same venue in two tabs is really in the room
 * they most recently entered.
 */
 ...
 * `event_venue_members` row is a 45s-throttled mirror and is only consulted for
 * members with no presence entry at all — for first paint before the channel
 * syncs, and for showing a teammate's status to someone who is not on this
 * venue's channel. A mirrored row older than VENUE.STALE_AFTER_MS renders as
 * 'offline' rather than as whatever it last said, because a two-hour-old
 * "working" is a lie.
 */
...
export function occupantsInRoom(
  ...
  return sortOccupants(occupants.filter((o) => o.room_id === roomId && o.availability !== 'offline'))
}
...
export function occupantsUnassigned(occupants: VenueOccupant[]): VenueOccupant[] {
  return sortOccupants(occupants.filter((o) => !o.room_id && o.availability !== 'offline'))
}
...
/**
 * What a floorplan zone draws: a few avatars and a count for the rest.
 * Capped because a zone is ~120px wide and 40 avatars in it is a smear.
 */
export function clusterForRoom(
  occupants: VenueOccupant[],
  roomId: string,
  visible: number = VENUE.CLUSTER_VISIBLE
): { shown: VenueOccupant[]; overflow: number; total: number } {
  const inRoom = occupantsInRoom(occupants, roomId)
  const cap = Math.max(0, visible)
  return {
    shown: inRoom.slice(0, cap),
    overflow: Math.max(0, inRoom.length - cap),
    total: inRoom.length,
  }
}
