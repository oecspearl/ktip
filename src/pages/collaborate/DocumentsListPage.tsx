...
import { Plus, Search, FileText, Trash2, ChevronRight, Users } from 'lucide-solid'
...
export default function DocumentsListPage() {
  ...
  const documents = useDocuments({ get search() { return searchQuery() } })
  ...
  const handleDelete = async (e: Event, docId: string) => {
    e.preventDefault()
    e.stopPropagation()
    ...
    <MainLayout>
      ...
      <div class="bg-gray-800 min-h-[140px] flex items-center">
        <div class="max-w-5xl mx-auto px-4 w-full flex items-center justify-between">
          <div>
            ...
            <h1 class="text-3xl md:text-4xl font-display font-bold text-white">My Documents</h1>
          </div>
          <nav class="hidden md:flex items-center gap-1 text-sm text-gray-400">
            ...
            <ChevronRight size={14} />
            <span class="text-gray-200">Documents</span>
          </nav>
        </div>
      </div>
      ...
      <div class="bg-white py-8">
        <div class="max-w-5xl mx-auto px-4">
          ...
          >
            <Show
              when={documents.documents() && documents.documents()!.length > 0}
              fallback={
                <div class="text-center py-16">
                  <div class="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-4">
                    <FileText size={32} class="text-gray-400" />
                  </div>
                  <h3 class="text-lg font-semibold text-ktip-sand-800 mb-1">No documents yet</h3>
            ...
            >
              <div class="space-y-2">
                <For each={documents.documents()}>
                  {(doc) => (
                    <A
                      href={`/collaborate/document/${doc.id}`}
          ...
          {/* Shared with me */}
          <Suspense>
            <Show when={shared.documents() && shared.documents()!.length > 0}>
              <div class="mt-10">
                  ...
                  <For each={shared.documents()}>
                    {(doc) => (
                      ...
                      >
                        ...
                        <span class="text-xs text-ktip-sand-400 px-2 py-1 bg-ktip-sand-50 rounded">View only</span>
                      </A>
                    )}
                  </For>
                </div>
